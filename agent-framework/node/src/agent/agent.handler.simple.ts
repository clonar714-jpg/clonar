...
import { validateAgentRequest, type AgentRequestBody } from "./agent.validation";
 ...
 */
export async function handleAgentRequestSimple(req: Request, res: Response): Promise<void> {
  try {
    // Validate request
    const validation = validateAgentRequest(req.body);
    if (!validation.success) {
      res.status(400).json(
        createErrorResponse('Invalid request body', validation.error)
      );
      return;
    }

    const body: AgentRequestBody = validation.data;
    const query = body.query.trim();

    if (!query || query.length === 0) {
      res.status(400).json(
        createErrorResponse('Query cannot be empty')
      );
      return;
    }

    // Build conversation history
    const conversationHistory = (body.conversationHistory || []).map((h: any) => ({
      query: h.query,
      summary: h.summary,
