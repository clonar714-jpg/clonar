 ...
 */
export function fuseCardsInOrder(intent, cards, sections) {
    ...
    if (sections && sections.length > 0) {
        const overviewSections = sections.filter(s => s.type === "overview" ||
            s.title?.toLowerCase().includes("overview") ||
            s.title?.toLowerCase().includes("summary"));
        const detailSections = sections.filter(s => s.type !== "overview" &&
            !s.title?.toLowerCase().includes("overview") &&
            !s.title?.toLowerCase().includes("summary"));
 ...
 */
export function mergeCardsIntelligently(primaryCards, secondaryCards, intent) {
