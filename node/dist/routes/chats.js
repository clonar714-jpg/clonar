...
router.get("/", async (req, res) => {
    try {
        ...
        const rawUserId = req.headers["user-id"] || "dev-user-id";
...
router.post("/:id/messages", async (req, res) => {
    try {
        ...
        if (convError || !existingConv) {
            ...
            const title = query?.substring(0, 100) || "New Conversation";
            try {
        ...
        const { query, summary, intent, cardType, cards, results, sections, answer, imageUrl, } = req.body;
