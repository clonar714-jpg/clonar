...
router.post('/multiple', skipAuthInDev(), uploadMultiple, processImage, handleUploadError, async (req, res) => {
    try {
...
router.post('/supabase', skipAuthInDev(), uploadSingle, processImage, handleUploadError, async (req, res) => {
    try {
        if (!req.file) {
            const response = {
                success: false,
...
router.delete('/:filename', skipAuthInDev(), async (req, res) => {
    try {
