...
router.get('/', optionalAuth, async (req, res) => {
    try {
        const { query, page = 1, limit = 20, sort_by = 'created_at', sort_order = 'desc', tags, user_id, is_published, layout, created_after, created_before, } = req.query;
...
router.post('/', skipAuthInDev(), async (req, res) => {
    try {
        ...
        const { title, description, cover_image_url, layout, settings, tags, is_published, items } = value;
        // ✅ Auto-set published if essential info is complete
        const autoPublished = title?.trim()?.length > 0 &&
            cover_image_url &&
...
router.put('/:id', skipAuthInDev(), async (req, res) => {
    try {
        ...
        if (value.items && Array.isArray(value.items)) {
            ...
            // Insert new items
            const itemsToInsert = value.items.map((item) => {
                let imageUrl = item.image_url || 'https://via.placeholder.com/100x100';
                if (item.type === 'text' && item.text) {
                    const textData = {
                        text: item.text,
...
// Add item to collage
router.post('/:id/items', skipAuthInDev(), async (req, res) => {
    try {
        // ✅ Validate user context
        if (!req.user || !req.user.id) {
            return res.status(401).json({ success: false, message: 'Unauthorized: No user context' });
...
// Update collage item
router.put('/:id/items/:itemId', skipAuthInDev(), async (req, res) => {
    try {
        // ✅ Validate user context
        if (!req.user || !req.user.id) {
            return res.status(401).json({ success: false, message: 'Unauthorized: No user context' });
