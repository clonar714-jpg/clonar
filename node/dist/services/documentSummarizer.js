 ...
 */
export async function summarizeDocument(content, query) {
 ...
 */
export async function summarizeDocuments(contents, query) {
    ...
    for (let i = 0; i < contents.length; i += BATCH_SIZE) {
        ...
        const batchResults = await Promise.all(batch.map((content) => summarizeDocument(content, query)));
