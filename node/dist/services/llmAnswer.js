 ...
 */
export async function searchWeb(query, conversationHistory = [], enableSummarization = true) {
 ...
 */
export function filterCompareAnswer(answerText, decisionAllowed = true, isContextualCompare = false) {
    if (!answerText)
        return answerText;
    ...
    if (!decisionAllowed) {
        decisionPhrases.push({ pattern: /\bbetter\b/gi, replacement: 'differs' }, { pattern: /\bbest\b/gi, replacement: 'notable' }, { pattern: /\bsuperior\b/gi, replacement: 'different' }, { pattern: /\bwinner\b/gi, replacement: 'option' }, { pattern: /\bshould choose\b/gi, replacement: 'could consider' }, { pattern: /\bideal\b/gi, replacement: 'suitable' });
    }
...
// NON-STREAMED ANSWER
// =======================================================================
export async function getAnswerNonStream(query, history, isClarificationOnly = false, confidenceBand, userGoal, answerPlan, answerSketch, // ✅ PERPLEXITY UPGRADE: Early answer commitment
reasoningLevel, // ✅ PERPLEXITY UPGRADE: Reasoning density control
reuseWebResults, // ✅ Improvement 2: Skip second web search on regeneration
responseIntent, // ✅ PERPLEXITY-STYLE: Intent for structured answer generation
dataRequirements) {
    ...
    let webResults = { snippets: [], sources: [] };
    if (reuseWebResults) {
            ...
            for (const h of history) {
                ...
                if (h.summary || h.answer) {
                    ...
                    if (h.cards && Array.isArray(h.cards) && h.cards.length > 0) {
                        const cardTitles = h.cards.slice(0, 5).map((card) => card.title || card.name || '').filter(Boolean);
...
// =======================================================================
export async function getAnswerStream(query, history, res, answerSketch, // ✅ PERPLEXITY UPGRADE: Early answer commitment
reasoningLevel, // ✅ PERPLEXITY UPGRADE: Reasoning density control
userGoal // ✅ COMPARE INVARIANT: For answer filtering
) {
