 ...
 */
function mapShoppingResultsToProductCards(shoppingResults) {
    if (!Array.isArray(shoppingResults) || shoppingResults.length === 0) {
        return [];
    }
    return shoppingResults.slice(0, 10).map((product, index) => {
...
function mapHotelResultsToHotelCards(hotelResults) {
    if (!Array.isArray(hotelResults) || hotelResults.length === 0) {
        return [];
    }
    return hotelResults.slice(0, 10).map((hotel, index) => ({
...
function mapPlaceResultsToPlaceCards(placeResults) {
    if (!Array.isArray(placeResults) || placeResults.length === 0) {
        return [];
    }
    return placeResults.slice(0, 10).map((place, index) => ({
 ...
 */
function mapGooglePlacesToPlaceCards(googlePlacesResults) {
    if (!Array.isArray(googlePlacesResults) || googlePlacesResults.length === 0) {
        return [];
    }
    return googlePlacesResults.slice(0, 10).map((place, index) => {
...
function mapMovieResultsToMovieCards(movieResults) {
    if (!Array.isArray(movieResults) || movieResults.length === 0) {
        return [];
    }
    return movieResults.slice(0, 10).map((movie) => ({
...
export function detectDomains(query) {
    ...
    if (/\b(movie|film|cinema|actor|director|watch|streaming|oscar|box office|trailer|cast|plot|review|imdb|rotten tomatoes)\b/.test(q) ||
    ...
    if (/\b(places? to visit|attractions?|landmarks?|things to do|sights?|tourist|monuments?|temples?|museums?|parks?|beaches?|nature)\b/.test(q) ||
    ...
    // Hotel detection
    if (/\b(hotel|accommodation|stay|lodging|resort|inn|motel|hostel|bed and breakfast|bnb)\b/.test(q) ||
        /\b(hotels? (in|near|at|around|to stay)|where to stay|book (a )?hotel)\b/.test(q)) {
        domains.push('hotel');
    }
    // Product/Shopping detection
    if ((/\b(buy|shop|purchase|price|\$|under|over|below|above|cheap|expensive|affordable)\b/.test(q) &&
    ...
    if (/\b(restaurant|dining|food|eat|cuisine|dinner|lunch|breakfast|brunch|cafe|bistro|bar|pub)\b/.test(q) ||
        /\b(restaurants? (in|near|at|around)|where to (eat|dine)|best (food|restaurant|pizza|sushi))\b/.test(q)) {
        domains.push('restaurant');
    }
    // Flight detection
    if (/\b(flight|flights?|airline|airport|fly|flying|ticket|booking|destination)\b/.test(q) ||
 ...
 */
async function searchWeb(query, conversationHistory = [], options) {
 ...
 */
async function summarizeDocuments(docs, query) {
    if (docs.length === 0)
        return [];
    console.log(`📝 Summarizing ${docs.length} documents...`);
    const summarized = await Promise.all(docs.map(async (doc) => {
        try {
...
async function rerankDocuments(query, docs) {
    ...
    const isSimpleQuery = (query.split(/\s+/).length <= 8 && // Short query
 ...
 */
function formatDocumentsAsContext(docs) {
    return docs
        .map((doc, index) => {
 ...
 */
async function generateFollowUpSuggestions(query, answerSummary) {
    try {
 ...
 */
async function generateAnswerStream(query, context, conversationHistory, res, sources) {
 ...
 */
export async function generatePerplexityAnswer(query, conversationHistory = [], shouldStream = false, res // Response object for streaming
) {
    try {
        ...
        const needsFreshness = (/\b(now|current|latest|recent|today|this week|this month|2024|2025)\b/i.test(query) ||
        ...
        const needsMultipleSources = (/\b(vs|versus|compare|comparison|difference between)\b/i.test(query) ||
        ...
        if (documents.length === 0) {
            ...
            const fallbackFollowUps = await generateFollowUpSuggestions(query, parsed.summary || parsed.answer.substring(0, 500));
            return {
        ...
        const followUpSuggestions = await generateFollowUpSuggestions(query, parsed.summary || parsed.answer.substring(0, 500));
        ...
        const filteredSections = parsed.sections.filter((section) => !section.title?.toUpperCase().includes('FOLLOW_UP_SUGGESTIONS'));
        ...
        if (shouldStream && res) {
            ...
            sse.send("end", {
                ...
                destination_images: allImages.map((img) => img.url),
                videos: allVideos,
