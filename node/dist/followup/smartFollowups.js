 ...
 */
async function removeSemanticDuplicates(list, last) {
    if (!last)
        return list;
 ...
 * SHOPPING FOLLOW-UPS
 */
function shoppingFollowUps(ctx) {
    const brand = ctx.brand ? ctx.brand.toUpperCase() : "";
    const cat = ctx.category || "products";
    const suggestions = [];
    if (brand) {
        suggestions.push(`Compare other ${cat} from ${brand}`, `Is ${brand} good for long-term durability?`, `Show alternatives to ${brand} ${cat}`);
    }
    suggestions.push(`Best ${cat} under higher price range`, `Which ${cat} offer the best comfort?`, `Compare materials & build quality`, `Which ${cat} are good for everyday use?`, `Show lightweight ${cat} options`);
    return suggestions;
}
/**
 * HOTEL FOLLOW-UPS
 */
function hotelFollowUps(ctx) {
 ...
 */
function restaurantFollowUps(ctx) {
    const city = ctx.city || "this area";
    return [
 ...
 * LOCATION FOLLOW-UPS
 */
function locationFollowUps(ctx) {
    const city = ctx.city || "this area";
    return [
 ...
 * (Perplexity-level)
 */
export async function generateSmartFollowUps(ctx) {
    let base = [];
    switch (ctx.intent) {
        case "shopping":
    ...
    if (ctx.lastFollowUp) {
        base = base.filter((s) => s.toLowerCase() !== ctx.lastFollowUp?.toLowerCase());
    }
