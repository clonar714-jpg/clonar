...
// ==================================================================
// Default empty state
export const initialBehaviorState = {
    lastIntent: null,
    lastCardType: null,
    lastBrand: null,
    lastCategory: null,
...
function detectInterestPattern(followUp) {
    const lower = followUp.toLowerCase();
    if (lower.includes("compare"))
        return "comparison";
    if (lower.includes("filter"))
        return "filtering";
    if (lower.includes("size") || lower.includes("color"))
        return "variants";
    ...
    if (lower.includes("available"))
        return "availability";
    if (lower.includes("long-distance") || lower.includes("running"))
        return "performance";
    return null;
}
// ==================================================================
...
// ==================================================================
export function updateBehaviorState(prev, params) {
    const next = { ...prev };
    next.lastIntent = params.intent || prev.lastIntent;
    next.lastCardType = params.cardType || prev.lastCardType;
    if (params.brand)
        next.lastBrand = params.brand;
    if (params.category)
        next.lastCategory = params.category;
    if (params.price)
        next.lastPrice = params.price;
    if (params.city)
        next.lastCity = params.city;
...
// ==================================================================
export function inferUserGoal(state) {
