...
function isWatchProduct(text) {
    ...
    if (shoppingTerms.some(term => lower.includes(term)))
        return true;
    return false;
}
/**
...
function detectCardType(text) {
    ...
    if (lower.includes("watch") || lower.includes("watches")) {
        if (isWatchMovie(lower))
            return "movies";
        if (isWatchProduct(lower))
            return "shopping";
    }
    // ✅ STEP 3: Check movies (with context)
    if (isWatchMovie(lower))
        return "movies";
    if (lower.includes("movie ticket") || lower.includes("cinema ticket") || lower.includes("theater ticket"))
        return "movies";
    if (MOVIE_KEYWORDS.some((k) => lower.includes(k)))
        return "movies";
    // ✅ STEP 4: Check shopping
    if (SHOPPING_KEYWORDS.some((k) => lower.includes(k)))
        return "shopping";
    // ✅ STEP 5: Check hotels
    if (HOTEL_KEYWORDS.some((k) => lower.includes(k)))
        return "hotel";
    // ✅ STEP 6: Check places (before restaurants)
    if (PLACES_KEYWORDS.some((k) => lower.includes(k)))
        return "places";
    // ✅ STEP 7: Check flights
    if (FLIGHT_KEYWORDS.some((k) => lower.includes(k)))
        return "flights";
    // ✅ STEP 8: Check restaurants
    if (RESTAURANT_KEYWORDS.some((k) => lower.includes(k)))
        return "restaurants";
    // ✅ STEP 9: Check location
    if (LOCATION_KEYWORDS.some((k) => lower.includes(k)))
        return "location";
    return "none";
}
function detectCardTrigger(text) {
    const lower = text.toLowerCase();
...
export function analyzeCardNeed(query) {
    return {
