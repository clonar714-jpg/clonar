 ...
 */
async function generateDetailContent(domain, title, description, additionalInfo) {
    const domainPrompts = {
        product: {
    ...
    try {
        ...
        try {
            ...
            parsed = {
                ...
                keyFeatures: lines.filter(l => l.trim().startsWith('-') || l.trim().startsWith('•')).slice(0, 5).map((l) => l.replace(/^[-•]\s*/, '')),
            };
        }
        return {
 ...
 */
async function fetchProductDetails(productId, title, link) {
 ...
 */
async function fetchHotelDetails(hotelId, name, link) {
    ...
    try {
        ...
        const hotel = hotels.find((h) => h.name === name || h.hotel_id === hotelId) || hotels[0];
        if (!hotel)
            return { images: [] };
        const images = [];
        if (hotel.thumbnail)
        ...
        if (hotel.images && Array.isArray(hotel.images)) {
            hotel.images.slice(0, 10).forEach((img) => {
 ...
 */
async function fetchPlaceDetails(placeId, name, link) {
 ...
 */
export async function handleDetailRequest(req, res) {
    try {
        ...
        if (!domain || !['product', 'hotel', 'place', 'movie'].includes(domain)) {
            res.status(400).json(createErrorResponse('Invalid domain. Must be: product, hotel, place, or movie'));
            return;
        }
        if (!title) {
        ...
        const detailContent = await generateDetailContent(domain, title, description || '', { ...additionalInfo, ...additionalDetails });
        res.json({
            success: true,
            domain,
            ...detailContent,
