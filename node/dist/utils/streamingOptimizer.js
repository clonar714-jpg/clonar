...
class StreamingOptimizer {
    constructor() {
        this.metrics = [];
        this.currentChunkSize = 50; // Initial chunk size (characters)
        this.minChunkSize = 10;
        this.maxChunkSize = 200;
        this.targetLatency = 100; // Target latency per chunk (ms)
    }
    /**
     * Record streaming metrics
     */
    recordMetrics(chunkSize, latency, throughput) {
        this.metrics.push({
            chunkSize,
            latency,
            throughput,
    ...
    optimizeChunkSize() {
        ...
        if (avgLatency > this.targetLatency * 1.5) {
            this.currentChunkSize = Math.max(this.minChunkSize, Math.floor(this.currentChunkSize * 0.9));
        ...
        // If latency is low, increase chunk size for better throughput
            ...
            this.currentChunkSize = Math.min(this.maxChunkSize, Math.floor(this.currentChunkSize * 1.1));
