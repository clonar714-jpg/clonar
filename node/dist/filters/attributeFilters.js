 ...
 */
export async function applyAttributeFilters(query, items) {
    ...
    try {
        ...
        const attrEmbeds = await Promise.all(attributes[targetKey].map((a) => getEmbedding(a)));
        ...
        const scored = await Promise.all(items.map(async (item) => {
            ...
            const maxSimilarity = Math.max(...attrEmbeds.map((emb) => cosine(qEmb, emb)), ...attrEmbeds.map((emb) => cosine(itemEmb, emb)));
