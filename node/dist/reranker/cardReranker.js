 ...
 */
function getAttributeBoosts(query, item) {
    let boost = 0;
 ...
 */
function applyCategoryPenalty(intent, item, baseScore) {
    let score = baseScore;
    if (intent === "shopping") {
 ...
 */
export async function rerankCards(query, items, intent) {
    ...
    const scored = await Promise.all(items.map(async (item) => {
        ...
        const itemPrice = extractPrice(item.price || "");
        const priceScore = priceLimit && itemPrice && itemPrice <= priceLimit ? 1 : 0;
    ...
    if (sorted.length > 0) {
        console.log(`🎯 Reranked ${sorted.length} items. Top 3 scores:`, sorted.slice(0, 3).map((s) => s.finalScore.toFixed(3)));
    }
    return sorted;
}
