...
function getDefaultWhatPeopleSay(name: string, location?: string, rating?: number): string {
...
function getDefaultReviewSummary(name: string, rating?: number, reviewCount?: number): string {
  const ratingText = rating && rating >= 4.5 
    ? "highly rated" 
    : rating && rating >= 4.0 
    ? "well-rated" 
...
function getDefaultChooseThisIf(name: string, amenities?: any[]): string {
...
function getDefaultAbout(description?: string, name?: string, amenities?: any[]): string {
...
function getDefaultLocationSummary(address?: string, location?: string, nearby?: string): string {
...
function getDefaultRatingInsights(cleanliness?: number, rooms?: number, service?: number, value?: number): string {
  const ratings = [];
  if (cleanliness) ratings.push({ name: 'cleanliness', value: cleanliness });
  if (rooms) ratings.push({ name: 'rooms', value: rooms });
  if (service) ratings.push({ name: 'service', value: service });
  if (value) ratings.push({ name: 'value', value: value });
