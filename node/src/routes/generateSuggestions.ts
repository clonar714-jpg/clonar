  ...
  try {
    ...
    if (safeResults.length === 0 && conversationHistory && conversationHistory.length > 0) {
      ...
      if (lastTurn.cards && Array.isArray(lastTurn.cards) && lastTurn.cards.length > 0) {
        ...
        safeResults = lastTurn.products.map((p: any) => ({
          title: p.title,
          price: p.price,
          source: p.source,
        }));
    ...
    if (safeResults.length > 0) {
      resultSummary = safeResults
        ...
        .map((item: any, i: number) => {
          return `${i + 1}. ${item.title || item.name || "Item"} - price: ${
            item.price || "N/A"
          }, source: ${item.source || item.website || "Unknown"}`;
        })
        .join("\n");
    }
...
`;
    ...
    try {
      ...
      try {
        ...
        suggestions = lines.slice(0, 3).map((line: string) => line.replace(/^[-*•]\s*/, "").trim());
      }
    } catch (err: any) {
