// src/services/query-rewrite.ts
// Query Rewrite module: normalize user language only. Rewrite is optional and gated.
// No reasoning, planning, decomposition, or intent detection. Context binding is handled separately.
...
/** Reasons for normalization (language cleanup and context resolution). */
export type NormalizeReason = 'typo' | 'ambiguity' | 'long_messy' | 'follow_up';

/** Reason when conflicting/unrealistic constraints are detected. */
export type RewriteReason = NormalizeReason | 'conflict';

/** Output when rewrite is NOT needed. */
export interface NoRewrite {
  rewriteNeeded: false;
}

/** Output when rewrite IS needed and can be safely normalized. */
...
/** Output when conflicting or unrealistic constraints are detected. */
...
/**
 * Run the gated query rewrite module. Returns spec-compliant JSON shape.
 * Use toRewriteResult() to adapt to the pipeline's RewriteResult.
 */
export async function normalizeQuery(ctx: QueryContext): Promise<QueryRewriteOutput> {
  ...
  // Perplexity-style: conversation thread from session so rewrite can resolve vague/follow-up queries from prior turns.
  ...
  // ChatGPT-style Memory: explicit facts + structured slots. Injected so we can resolve "my birthday", "where I live", etc.
...
/** Adapter: map QueryRewriteOutput to the pipeline's RewriteResult (query-understanding + orchestrator). */
export function toRewriteResult(
  ...
  // needsClarification: true, normalizedQuery null — use conflicts only; do not put conflicts in alternatives (alternatives = alternative phrasings for UI).
  return {
...
/** Result of rewrite-only step. No vertical or intent — the pipeline decides those elsewhere. */
...
/**
 * Step 4 rewrite only: normalize query and return the rewritten text + metadata.
 * Does NOT set vertical or intent; the orchestrator builds the minimal plan (vertical: 'other', intent: 'browse') from this result.
 */
