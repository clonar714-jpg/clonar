...
Be specific and detailed. If it's a product, describe it like a shopping search. If it's a place/hotel, describe it like a travel search.`,
    ...
    const visionTimeoutPromise = new Promise<never>((_, reject) => 
      setTimeout(() => reject(new Error('OpenAI Vision API timeout after 30 seconds')), 30000)
    );
    ...
    try {
      ...
      let description = "";
      let keywords: string[] = [];
      let enhancedQuery: string | undefined = undefined;
 ...
 */
export async function enhanceQueryWithImage(
  textQuery: string,
  imageUrl: string
): Promise<string> {
  try {
