...
const HIGH_CONFIDENCE_PATTERNS = {
  ...
  priceModifiers: {
    ...
    confidence: 0.95,
    extractModifiers: (query: string): string[] => {
      const modifiers: string[] = [];
      if (/\b(luxury|premium|high-end|expensive|costly)\b/i.test(query)) modifiers.push('luxury');
      if (/\b(cheap|cheaper|affordable|budget|economy|low-cost)\b/i.test(query)) modifiers.push('budget');
      return modifiers;
    },
  },
  ...
  starRatings: {
    ...
    confidence: 0.95,
    extractPrice: (query: string): string | null => {
 ...
 */
export function extractContextWithRules(
  query: string,
  parentQuery?: string | null
): RefinementResult | null {
  const lowerQuery = query.toLowerCase().trim();
  let maxConfidence = 0;
  let bestMatch: ExtractedContext | null = null;
 ...
 */
function extractLocationFromQuery(query: string): boolean {
 ...
 */
export function mergeQueryWithRules(
  currentQuery: string,
  parentQuery: string,
  extractedContext: ExtractedContext,
  intent: string
