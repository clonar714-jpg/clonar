...
import { ShopifyProvider } from "./providers/exampleProviders";
...
// =======================================================================
// 🚀 C6 PATCH #1 — 3-Layer Retry System Helpers
// =======================================================================
...
async function tryFetch(fn: () => Promise<any[]>, label: string): Promise<SearchAttempt> {
  try {
...
function mergeCardResults(attempts: SearchAttempt[]): any[] {
  const merged: any[] = [];
 ...
 */
async function searchWithProviders(query: string, options?: any): Promise<any[]> {
 ...
 */
export async function enrichProductsWithDescriptions(products: any[]): Promise<void> {
  ...
  const descriptionPromises = products.map(async (product) => {
      try {
        ...
        if (product.extensions && Array.isArray(product.extensions)) {
          features.push(...product.extensions.map((e: any) => String(e || '')));
        }
        if (product.tag) features.push(product.tag);
        
        // Extract materials/design hints from snippet
        const materials: string[] = [];
        const rawSnippet = product._raw_snippet || product.snippet || '';
        if (rawSnippet.toLowerCase().includes('mesh')) materials.push('mesh');
        if (rawSnippet.toLowerCase().includes('leather')) materials.push('leather');
        if (rawSnippet.toLowerCase().includes('rubber')) materials.push('rubber');
        if (rawSnippet.toLowerCase().includes('foam')) materials.push('foam');
        
        // Infer category from title/snippet
        let category = '';
        const titleLower = (product.title || '').toLowerCase();
        const snippetLower = rawSnippet.toLowerCase();
        if (titleLower.includes('running') || snippetLower.includes('running')) category = 'Running';
        else if (titleLower.includes('basketball') || snippetLower.includes('basketball')) category = 'Basketball';
        else if (titleLower.includes('lifestyle') || snippetLower.includes('lifestyle')) category = 'Lifestyle';
        else if (titleLower.includes('sneaker') || snippetLower.includes('sneaker')) category = 'Sneakers';
        else if (titleLower.includes('shoe')) category = 'Footwear';
 ...
 */
async function fallbackShoppingAPI(query: string): Promise<any[]> {
