// src/services/eval-automated.ts
// Automated component-level evals (no LLM): rewrite quality, filter appropriateness, routing correctness.
// Run in-process after a request or in a batch job over sampled payloads.
import { tokenize } from './providers/retrieval-vector-utils';

/** 0–1 score: rewrite preserved intent and added useful context (heuristic: token overlap + length change). */
...
/** 0–1 score: filters are appropriate for the query (heuristic: key filter terms appear in query or rewritten). */
...
/** 0–1 score: routing was correct (heuristic: primary route matches the vertical that had the most retrieval results). */
...
/** Run all automated evals for one request (for logging or metrics callback). */
