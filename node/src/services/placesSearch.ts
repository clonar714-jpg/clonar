...
// ✅ PHASE 3: In-memory cache for geocoding and place details
interface CacheEntry<T> {
  data: T;
  timestamp: number;
}

const GEOCODE_CACHE = new Map<string, CacheEntry<{ latitude: number; longitude: number }>>();
const PLACE_DETAILS_CACHE = new Map<string, CacheEntry<{
  image: string | null;
  images: string[];
  website: string | null;
  phone: string | null;
...
function getCacheKey(placeName: string, location: string): string {
...
async function getPlaceDetails(
  placeName: string, 
  location: string, 
  needWebsite: boolean = false, 
  needPhone: boolean = false,
  needImages: boolean = true // ✅ FIX: Always fetch images by default to get all photos
): Promise<{
  image: string | null;
  images: string[];
  website: string | null;
  ...
  try {
    ...
    if (searchResponse.data.status === 'OK' && searchResponse.data.results && searchResponse.data.results.length > 0) {
      ...
      if (place.place_id && (needWebsite || needPhone || needImages)) {
        ...
        const fields: string[] = ['photos']; // ✅ FIX: Always request photos to get all images
        if (needWebsite) {
          fields.push('website');
        }
        if (needPhone) {
