...
const PRICE_PATTERNS = [
  { pattern: /\$(\d+)\s*-\s*\$(\d+)/, extract: (match: RegExpMatchArray) => ({ min: parseInt(match[1]), max: parseInt(match[2]) }) },
  { pattern: /under\s*\$(\d+)/i, extract: (match: RegExpMatchArray) => ({ max: parseInt(match[1]) }) },
  { pattern: /above\s*\$(\d+)/i, extract: (match: RegExpMatchArray) => ({ min: parseInt(match[1]) }) },
  { pattern: /(\d+)\s*-\s*(\d+)\s*dollars/i, extract: (match: RegExpMatchArray) => ({ min: parseInt(match[1]), max: parseInt(match[2]) }) },
...
const RATING_PATTERNS = [
  { pattern: /(\d+)\s*[-*]?\s*star/i, extract: (match: RegExpMatchArray) => parseInt(match[1]) },
  { pattern: /(\d+)\s*out\s*of\s*(\d+)/i, extract: (match: RegExpMatchArray) => parseFloat(match[1]) / parseFloat(match[2]) * 5 },
];
 ...
 */
export function extractStyleKeywords(query: string, cards: any[] = []): string[] {
  const found = new Set<string>();
  const text = (
    query + " " + 
    cards.map(c => c.title || c.name || "").join(" ") +
    " " +
    cards.map(c => c.description || "").join(" ")
  ).toLowerCase();
 ...
 */
export function extractPriceRange(query: string, cards: any[] = []): { min?: number; max?: number } | null {
 ...
 */
export function extractBrands(query: string, cards: any[] = []): string[] {
 ...
 */
export function extractRatings(query: string, cards: any[] = []): string[] {
  const found: string[] = [];
