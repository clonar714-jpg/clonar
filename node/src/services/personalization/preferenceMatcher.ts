...
export interface PreferenceProfile {
  text: string; // Combined text description of preferences
 ...
 */
export async function matchItemsToPreferences(
  items: any[],
  userId: string,
  intent: string,
  category?: string
      ...
      items.map(async (item) => {
        ...
        if (intent === "shopping") {
          ...
          itemText = [
            ...
            item.genres ? (Array.isArray(item.genres) ? item.genres.map((g: any) => g.name || g).join(" ") : item.genres) : "",
        ...
        if (intent === "shopping" && userPrefs.brand_preferences && userPrefs.brand_preferences.length > 0) {
          ...
          const hasBrand = userPrefs.brand_preferences.some(brand =>
            itemTextLower.includes(brand.toLowerCase())
          );
          if (hasBrand) {
        ...
        if (userPrefs.style_keywords && userPrefs.style_keywords.length > 0) {
          const itemTextLower = itemText.toLowerCase();
          const hasStyle = userPrefs.style_keywords.some(style =>
            itemTextLower.includes(style.toLowerCase())
          );
          if (hasStyle) {
...
export async function hybridRerank(
  ...
  try {
    ...
    // Load user preferences
    let profileEmb: number[] | null = null;
    if (userId && userId !== "global" && userId !== "dev-user-id") {
      const userPrefs = await getUserPreferences(userId);
