...
export interface EnhancementOptions {
  intent: string;
  category?: string;
  minConfidence?: number; // Minimum confidence score to apply preferences (default: 0.3)
}
 ...
 */
export async function enhanceQueryWithPreferences(
  query: string,
  userId: string,
  ...
  try {
    ...
    if (options.intent === "shopping" && userPrefs.brand_preferences && userPrefs.brand_preferences.length > 0) {
      ...
      ) || commonBrands.some(brand => queryLower.includes(brand));
 ...
 */
export function extractCategoryFromQuery(query: string, intent: string): string | undefined {
  const queryLower = query.toLowerCase();
