...
export type FieldType = 'shopping' | 'hotels' | 'flights' | 'restaurants' | 'places' | 'movies';

export interface BaseProvider<TResult = any> {
  name: string;
  fieldType: FieldType;
  search(query: string, options?: SearchOptions): Promise<TResult[]>;
}
...
export class QueryOptimizer {
   ...
   */
  static optimize(query: string, fieldType: FieldType): string {
  ...
  private static optimizeShopping(query: string): string {
  ...
  private static optimizeHotels(query: string): string {
  ...
  private static optimizeFlights(query: string): string {
 ...
 * Works for all field types
 */
export function extractFilters(query: string, fieldType: FieldType): Record<string, any> {
 ...
 */
export function applyBackendFilters<T extends Record<string, any>>(
  results: T[],
  filters: Record<string, any>,
  fieldType: FieldType
): T[] {
