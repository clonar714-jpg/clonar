...
export interface PlacesSectionResponse {
  sections: PlacesSection[];
}
 ...
 */
export function buildPlacesSectionPrompt(countryOrCity: string): string {
  return `
...
export async function generatePlacesSections(
  countryOrCity: string
): Promise<PlacesSectionResponse> {
  try {
    ...
    const parsed = JSON.parse(cleaned) as PlacesSectionResponse;
...
function getDefaultSections(): PlacesSectionResponse {
  return {
    sections: [
