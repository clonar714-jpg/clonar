 ...
 * @returns Confidence score (0-1)
 */
export function computeSlotConfidence(slots: Slots, intent: string): number {
  ...
  if (lowerIntent === "shopping") {
    if (slots.category) confidence += 0.2;
    if (slots.brand) confidence += 0.2;
    if (slots.priceRange) confidence += 0.2;
  } else if (lowerIntent === "hotels" || lowerIntent === "flights") {
    if (slots.dates) confidence += 0.3;
    if (slots.peopleCount) confidence += 0.2;
  } else if (lowerIntent === "restaurants") {
    if (slots.intentSpecific?.cuisine) confidence += 0.2;
    if (slots.intentSpecific?.mealType) confidence += 0.1;
  }
 ...
 * @returns Confidence score (0-1)
 */
export function computeCardConfidence(card: any, intent: string): number {
  ...
  // ✅ Essential fields
  if (card.title || card.name) confidence += 0.2;
  if (card.description) confidence += 0.1;

  // ✅ Intent-specific fields
  if (lowerIntent === "shopping") {
    if (card.price) confidence += 0.2;
    if (card.brand) confidence += 0.1;
    if (card.rating) confidence += 0.1;
    if (card.images && Array.isArray(card.images) && card.images.length > 0) {
      confidence += 0.1;
    }
  } else if (lowerIntent === "hotels") {
    if (card.rating || card.overall_rating) confidence += 0.2;
    if (card.address) confidence += 0.1;
    if (card.latitude && card.longitude) confidence += 0.1;
    if (card.amenities && Array.isArray(card.amenities)) {
      confidence += 0.1;
    }
  } else if (lowerIntent === "restaurants") {
    if (card.rating) confidence += 0.2;
    if (card.cuisine) confidence += 0.1;
    if (card.address) confidence += 0.1;
  } else if (lowerIntent === "places") {
    if (card.description) confidence += 0.2;
    if (card.latitude && card.longitude) confidence += 0.1;
    if (card.rating) confidence += 0.1;
  } else if (lowerIntent === "flights") {
    if (card.airline) confidence += 0.2;
    if (card.departure && card.arrival) confidence += 0.2;
    if (card.price) confidence += 0.1;
  }
