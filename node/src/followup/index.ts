...
import { inferIntentStage, IntentStage } from "./intentStage";
import { detectAnswerCoverage, AnswerCoverage } from "./answerCoverage";
...
export async function getFollowUpSuggestions(params: {
  ...
  answerPlan?: any; // ✅ ANSWER-FIRST: Pass answer plan for context
}) {
  ...
  const rankedWithScores = await rerankFollowUps(
    query,
    filteredCombined,
    5,
    answerSummary,
  ...
  let allCandidates: Array<{ candidate: string; embeddingScore: number }> = rankedWithScores.map((r) => ({
