...
import { generateSmartFollowUps } from "../utils/smartFollowups";
...
export async function generateFollowUps(input: {
  ...
  answer: string;
  intent: string;
  prevBehaviorState: BehaviorState;
  ...
  const cardAnalysis: CardAnalysisResult = analyzeCardNeed(query);
  ...
  const semanticMatches = await findClosestTemplates(
    ctxEmbedding,
    10,
    cardAnalysis.cardType
  );
  ...
  if (lastFollowUp) {
    filtered = filtered.filter(
      (s) => s.toLowerCase().trim() !== lastFollowUp.toLowerCase().trim()
    );
  }
  ...
  if (parentQuery) {
    filtered = filtered.filter(
      (s) => s.toLowerCase().trim() !== parentQuery.toLowerCase().trim()
    );
  }
  ...
  if (lastFollowUp && filtered.length > 0) {
    try {
      ...
      const filteredBySimilarity: string[] = [];
