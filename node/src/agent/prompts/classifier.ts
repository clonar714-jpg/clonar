...
7. showCalculationWidget (boolean): Decide if displaying a calculation widget would adequately address the user's query.
   ...
   - If it can fully answer the user query without needing additional search, set skipSearch to true as well.
8. showProductWidget (boolean): Decide if displaying a product/shopping widget would adequately address the user's query.
   - Set it to true if the user's query is about products, shopping, prices, or purchasing items.
   - Set it to true for queries like "best running shoes", "iPhone price", "laptop under $1000", "where to buy [product]".
   - Products usually need research for context, so skipSearch should typically be false.
9. showHotelWidget (boolean): Decide if displaying a hotel widget would adequately address the user's query.
   - Set it to true if the user's query is about hotels, accommodations, or booking stays.
   - Set it to true for queries like "hotels in Miami", "best hotels in Paris", "cheap hotels near me", "luxury hotels in New York".
   - Hotels usually need research for reviews/context, so skipSearch should typically be false.
10. showPlaceWidget (boolean): Decide if displaying a place/restaurant widget would adequately address the user's query.
   - Set it to true if the user's query is about places, restaurants, cafes, stores, or nearby locations.
   - Set it to true for queries like "restaurants near me", "best cafes in Seattle", "stores in downtown", "places to visit in Tokyo".
   - Places usually need research for details, so skipSearch should typically be false.
11. showMovieWidget (boolean): Decide if displaying a movie widget would adequately address the user's query.
   - Set it to true if the user's query is about movies, films, or cinema.
   - Set it to true for queries like "best movies 2024", "Inception movie", "where to watch [movie]", "movie showtimes".
   - Movies usually need research for reviews/context, so skipSearch should typically be false.
</labels>
...
<output_format>
You must respond in the following JSON format. Return ONLY valid JSON without any extra text, explanations or filler sentences:
{
  "classification": {
    "skipSearch": boolean,
