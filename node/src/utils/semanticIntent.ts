...
const embeddingExamples: Record<IntentType, string[]> = {
  shopping: [
 ...
 */
function hasProductCategory(query: string): boolean {
  const lower = query.toLowerCase();
 ...
 */
function isWatchMovieContext(query: string): boolean {
  const lower = query.toLowerCase();
...
function isWatchProductContext(query: string): boolean {
  ...
  if (productIndicators.some(term => lower.includes(term))) return true;
 ...
 */
function isMovieTicketContext(query: string): boolean {
  const lower = query.toLowerCase();
 ...
 */
function isFlightTicketContext(query: string): boolean {
  const lower = query.toLowerCase();
  if (!lower.includes("ticket")) return false;
...
function fastKeywordIntent(q: string): IntentType | null {
  ...
  if (hasBrand(lower)) {
    ...
    if (isWatchMovieContext(lower)) return "movies";
    return "shopping";
  }
  ...
  if (hasProductCategory(lower)) {
    ...
    if (lower.includes("watch") || lower.includes("watches")) {
      if (isWatchMovieContext(lower)) return "movies";
      if (isWatchProductContext(lower)) return "shopping";
      // Default: if has shopping action terms, it's shopping
      if (SHOPPING_ACTION_TERMS.some(term => lower.includes(term))) return "shopping";
    } else {
  ...
  if (isMovieTicketContext(lower)) return "movies";
  if (isWatchMovieContext(lower)) return "movies";
  if (MOVIE_TERMS.some(k => lower.includes(k))) return "movies";
  
  // ✅ STEP 4: Check shopping (after context checks)
  if (SHOPPING_TERMS.some(k => lower.includes(k))) return "shopping";
  
  // ✅ STEP 5: Check hotels
  if (q.includes("hotels")) return "hotels";
  if (HOTEL_TERMS.some(k => lower.includes(k))) return "hotels";
  
  // ✅ STEP 6: Check places (before restaurants)
  if (PLACES_TERMS.some(k => lower.includes(k))) return "places";
  
  // ✅ STEP 7: Check restaurants
  if (RESTAURANT_TERMS.some(k => lower.includes(k))) return "restaurants";
  
  // ✅ STEP 8: Check flights (with ticket context)
  if (isFlightTicketContext(lower)) return "flights";
  if (FLIGHT_TERMS.some(k => lower.includes(k))) return "flights";
  
  // ✅ STEP 9: Other intents
  if (LOCATION_TERMS.some(k => lower.includes(k))) return "location";
  if (IMAGE_TERMS.some(k => lower.includes(k))) return "images";
  if (LOCAL_TERMS.some(k => lower.includes(k))) return "local";
...
async function llmIntentClassifier(query: string): Promise<IntentType> {
  const system = `
...
`;
  ...
  try {
    ...
    const intent = (res.choices[0]?.message?.content ?? "").trim().toLowerCase() as IntentType;
    const valid: IntentType[] = [
      "shopping","hotels","flights","restaurants","places","movies",
      "location","general","images","local","answer"
    ];
    if (!valid.includes(intent)) return "general";
    return intent;
  } catch {
    return "general";
  }
...
async function embeddingIntent(query: string): Promise<IntentType> {
  const qEmb = await getEmbedding(query);
  let best: { intent: IntentType; score: number } = { intent: "general", score: 0 };

  for (const intent of Object.keys(embeddingExamples) as IntentType[]) {
...
export async function detectSemanticIntent(query: string): Promise<IntentType> {
...
export async function shouldFetchCards(query: string, answer: string): Promise<IntentType | null> {
...
export async function semanticIntent(q: string): Promise<IntentType> {
