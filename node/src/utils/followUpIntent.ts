...
// ------------------------------------------------------------------
async function matchCategory(query: string, list: string[]): Promise<number> {
 ...
 */
function queryIsWeak(query: string): boolean {
  const weakPatterns = [
    "best ones",
    "what colors?",
    "what color?",
...
export async function detectFollowUpIntent(
  ...
  if (previousIntent === "places") {
    ...
    if (placesScore >= 0.35) return "places";
    return explicit;
  }
...
// ------------------------------------------------------------------
export async function detectFollowUpCardNeed(
  query: string,
  conversationHistory: any[]
): Promise<{ needsCards: boolean; cardType: 'shopping' | 'hotel' | 'flights' | 'restaurants' | null }> {
  try {
    let previousIntent: string = 'general';
    ...
    const cardType = needsCards ? (detectedIntent as 'shopping' | 'hotel' | 'flights' | 'restaurants') : null;
