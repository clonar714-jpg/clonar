...
export function rankFollowUps(
  ...
  const scored = filtered.map(followUp => {
    ...
    if (intent === "shopping") {
      if (lower.includes("price") || lower.includes("cost")) score += 3;
      if (lower.includes("compare") || lower.includes("alternative")) score += 2;
      if (lower.includes("review") || lower.includes("rating")) score += 2;
      if (lower.includes("buy") || lower.includes("purchase")) score += 1;
    } else if (intent === "hotels") {
      if (lower.includes("map") || lower.includes("location")) score += 3;
      if (lower.includes("amenity") || lower.includes("feature")) score += 2;
      if (lower.includes("deal") || lower.includes("discount")) score += 2;
      if (lower.includes("nearby") || lower.includes("attraction")) score += 2;
    } else if (intent === "places") {
      if (lower.includes("map") || lower.includes("location")) score += 3;
      if (lower.includes("how to") || lower.includes("get there")) score += 2;
      if (lower.includes("best time") || lower.includes("when")) score += 2;
      if (lower.includes("tour") || lower.includes("guide")) score += 2;
    } else if (intent === "restaurants") {
      if (lower.includes("menu") || lower.includes("dish")) score += 3;
      if (lower.includes("review") || lower.includes("rating")) score += 2;
      if (lower.includes("reservation") || lower.includes("book")) score += 2;
      if (lower.includes("cuisine") || lower.includes("type")) score += 2;
    } else if (intent === "flights") {
      if (lower.includes("price") || lower.includes("deal")) score += 3;
      if (lower.includes("time") || lower.includes("schedule")) score += 2;
      if (lower.includes("airline") || lower.includes("route")) score += 2;
    }

    // ✅ Slot relevance scoring
    if (slots) {
      if (slots.location || slots.city) {
        if (lower.includes("nearby") || lower.includes("around")) score += 1;
      }
      if (slots.category) {
        if (lower.includes(slots.category.toLowerCase())) score += 1;
      }
      if (slots.brand) {
        if (lower.includes(slots.brand.toLowerCase())) score += 1;
      }
    }
