...
/** Heuristic strength of last result (for planner/personalization; no ML). */
export type ResultStrength = 'weak' | 'ok' | 'strong';
...
export interface SessionState {
  ...
  /** Last vertical that returned results (for UI/planner). */
  lastSuccessfulVertical?: Vertical;
  /** Heuristic strength of last result: weak | ok | strong. */
  lastResultStrength?: ResultStrength;
}
...
export interface UpdateSessionUpdates {
  ...
  lastMovieFilters?: PlanCandidateMovieFilters;
  lastProductFilters?: PlanCandidateProductFilters;
  /** Last vertical that returned results (evaluative; for next-turn context). */
  lastSuccessfulVertical?: Vertical;
  /** Heuristic strength of last result: weak | ok | strong. */
  lastResultStrength?: ResultStrength;
}
...
export async function updateSession(sessionId: string, updates: UpdateSessionUpdates): Promise<void> {
  ...
  const lastSuccessfulVertical = updates.lastSuccessfulVertical ?? existing?.lastSuccessfulVertical;
  const lastResultStrength = updates.lastResultStrength ?? existing?.lastResultStrength;
  ...
  await saveSession(sessionId, {
    ...
    ...(lastSuccessfulVertical != null && { lastSuccessfulVertical }),
    ...(lastResultStrength != null && { lastResultStrength }),
  });
}
